...
5. **Children** - pick relationships on this OS, then columns on that object. You can hop deeper (same idea as builder child options). Breadcrumb shows `Children / ASSET / SITE`. x trims the trail.
...
8. **Child row filters** - WHERE on a child path (not only parent WHERE). Each hop also has a nested row **limit** (default 50) so the MCP server does not silently cap the fetch. **No cap** sends `noLimit`.
