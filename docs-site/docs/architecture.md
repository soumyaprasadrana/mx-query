...
`GET /api/version` returns product name, mxQuery semver, and the pinned MCP npm spec:
...
No git SHA, environment, or runtime versions. The UI header reads this on boot (`v1.2.0` and `mcp v1.4.6`).
