...
`MQB_SESSION_ENCRYPTION_KEY` must be set here, and match what the running
server container uses. Without it, the app falls back to a key derived
from the container's own hostname - and this CLI invocation is a
different container than your running server, with a different random
hostname, so it would encrypt this tenant's API key with a key the server
can never reproduce to decrypt it. See [Install](/getting-started) for the
full explanation.
